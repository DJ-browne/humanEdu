	...
	<% } else { %>
		...
		<% for ( BoardRec rec : mList ) { %>
			...
			<td width="7%" align="center" valign="middle" height="25">
				<% if ( rec.getLevel() == 0) { %>
					<a href="BoardView.jsp?id=<%=rec.getArticleId() %>"> <%=rec.getArticleId() %> </a> 
				<% } %>
			</td>
			<td width="49%" align="left" valign="middle" height="25">
				...
				<% } %>
				<% if ( rec.getLevel() != 0) { %>
					<img alt="이미징" src="imgs/board_re.gif">
				<% } %>
				<a href="BoardView.jsp?id=<%=rec.getArticleId() %>"><%=rec.getTitle() %> </a> 
			</td>
			<td width="14%" align="center" valign="middle" height="25"><%= rec.getWriterName() %></td>
			<td width="14%" align="center" valign="middle" height="25"><%= rec.getPostingDate() %></td>
			<td width="8%" align="center" valign="middle" height="25"><%= rec.getReadCount() %></td>                
			</tr>
		<% } // end for %>
